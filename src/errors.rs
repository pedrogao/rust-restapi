use actix_web::{
    error::{BlockingError, ResponseError, JsonPayloadError as WebJsonPayloadError},
    http::StatusCode,
    HttpResponse,
};
...
pub enum ApiError {
    ...
    #[display(fmt = "Unauthorized! Please check you identity")]
    Unauthorized(String),
    // Payload size is bigger than allowed.
    PayloadOverflow(String),
    // Content type error, Deserialize error, Payload error
...
impl ResponseError for ApiError {
    fn error_response(&self) -> HttpResponse {
        match self {
            ...
            ApiError::JsonPayloadError(error) => {
                HttpResponse::BadRequest().json::<ErrorResponse>(error.into())
            }
            ApiError::PayloadOverflow(error) => {
                HttpResponse::PayloadTooLarge().json::<ErrorResponse>(error.into())
            }
...
/// Utility to make transforming a string into an ErrorResponse
impl From<String> for ErrorResponse {
    fn from(error: String) -> Self {
        ErrorResponse {
            errors: vec![error.into()],
...
/// Convert JsonPayloadError to ApiErrors
impl From<WebJsonPayloadError> for ApiError {
    fn from(error: WebJsonPayloadError) -> ApiError {
        match error {
            WebJsonPayloadError::Overflow => {
                ApiError::PayloadOverflow(error.to_string())
            }
            _ => ApiError::JsonPayloadError(error.to_string()),
        }
    }
}
